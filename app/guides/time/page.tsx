import type { Metadata } from "next";
import { Header } from "@/components/layout/Header";
import { Footer } from "@/components/layout/Footer";
import Link from "next/link";
import { Clock, ArrowRight, Calendar, Sparkles, Hourglass } from "lucide-react";

export const metadata: Metadata = {
  title: "Date & Time Guides | Time Calculation Tutorials | Calculatify",
  description:
    "Master date and time calculations with our comprehensive guides. Learn date math, time differences, leap years, and more with step-by-step tutorials.",
  keywords: ["date calculation guide", "time calculation", "date math", "time difference", "calendar calculations"],
};

const guides = [
  {
    title: "How to Calculate Dates and Time Differences Like a Pro",
    description:
      "Master date calculations with our comprehensive guide. Learn how to calculate days between dates, understand leap years, handle time zones, and solve real-world date problems.",
    href: "/guides/time/date-calculation",
    icon: Calendar,
    readTime: "12 min read",
    category: "Date Math",
  },
];

const comingSoon = [
  {
    title: "Age Calculation Methods",
    description: "Different ways to calculate age across cultures and systems.",
    icon: Hourglass,
  },
];

export default function TimeGuidesPage() {
  return (
    <div className="flex min-h-screen flex-col">
      <Header />

      <main className="flex-1 bg-white">
        {/* Hero Section */}
        <section className="bg-slate-50 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-4xl text-center">
            <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-blue-100 px-4 py-2 text-sm font-medium text-blue-700">
              <Clock className="h-4 w-4" />
              Date & Time Category
            </div>
            <h1 className="mb-4 text-4xl font-bold tracking-tight text-slate-900 sm:text-5xl">
              Date & Time Guides
            </h1>
            <p className="mx-auto max-w-2xl text-lg text-slate-600">
              Master time calculations with our comprehensive guides. From date
              arithmetic to time zones, learn practical skills for scheduling
              and planning.
            </p>
          </div>
        </section>

        {/* Breadcrumb */}
        <div className="border-b border-slate-200 bg-white px-4 py-3 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-6xl">
            <nav className="flex items-center gap-2 text-sm text-slate-500">
              <Link href="/guides" className="hover:text-indigo-600">
                All Guides
              </Link>
              <span>/</span>
              <span className="font-medium text-slate-900">Date & Time</span>
            </nav>
          </div>
        </div>

        {/* Guides Grid */}
        <section className="px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-6xl">
            <div className="mb-8 flex items-center gap-2">
              <Sparkles className="h-5 w-5 text-blue-600" />
              <h2 className="text-xl font-bold text-slate-900">
                Featured Guides ({guides.length})
              </h2>
            </div>

            <div className="grid gap-6 md:grid-cols-2">
              {guides.map((guide) => (
                <Link
                  key={guide.href}
                  href={guide.href}
                  className="group flex flex-col rounded-xl border border-slate-200 bg-white p-6 transition-all hover:-translate-y-1 hover:shadow-lg"
                >
                  <div className="mb-4 flex items-start justify-between">
                    <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-100">
                      <guide.icon className="h-6 w-6 text-blue-600" />
                    </div>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
                      {guide.category}
                    </span>
                  </div>

                  <h3 className="mb-2 text-xl font-bold text-slate-900 group-hover:text-blue-600">
                    {guide.title}
                  </h3>

                  <p className="mb-4 flex-1 text-slate-600">{guide.description}</p>

                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                      <Clock className="h-4 w-4" />
                      {guide.readTime}
                    </div>
                    <span className="flex items-center gap-1 text-sm font-medium text-blue-600 group-hover:gap-2 transition-all">
                      Read Guide
                      <ArrowRight className="h-4 w-4" />
                    </span>
                  </div>
                </Link>
              ))}
            </div>

            {/* Coming Soon */}
            {comingSoon.length > 0 && (
              <div className="mt-12">
                <h3 className="mb-4 text-lg font-semibold text-slate-900">
                  Coming Soon
                </h3>
                <div className="grid gap-4 md:grid-cols-2">
                  {comingSoon.map((item) => (
                    <div
                      key={item.title}
                      className="flex items-start gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4 opacity-75"
                    >
                      <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-200">
                        <item.icon className="h-5 w-5 text-slate-500" />
                      </div>
                      <div>
                        <h4 className="font-medium text-slate-700">
                          {item.title}
                        </h4>
                        <p className="text-sm text-slate-500">
                          {item.description}
                        </p>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            )}
          </div>
        </section>

        {/* Related Tools CTA */}
        <section className="border-t border-slate-200 bg-slate-50 px-4 py-16 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="mb-4 text-2xl font-bold text-slate-900">
              Practice What You Learned
            </h2>
            <p className="mb-6 text-slate-600">
              Try our free date and time calculators to apply your new skills.
            </p>
            <Link
              href="/tools/time"
              className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-6 py-3 text-sm font-medium text-white transition-colors hover:bg-blue-700"
            >
              Explore Date & Time Calculators
              <ArrowRight className="h-4 w-4" />
            </Link>
          </div>
        </section>
      </main>

      <Footer />
    </div>
  );
}
